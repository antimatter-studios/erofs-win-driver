//! EROFS mount handle and (with the `mount` feature) WinFsp adapter.
//!
...
//! no verbatim copies of winfsp-rs example filesystems or erofs-utils.
...
///     EROFS reader a partition slice without it knowing about the
///     surrounding disk.
///   - Reports `len` as the device size, so `BlockRead::size_bytes`
///     correctly reflects the slice's length (used by the EROFS
...
/// presses Ctrl-C / dismounts. EROFS is read-only at the format level,
...
pub enum DismountPolicy {
    ...
    Sidecar(PathBuf),
    /// Walk the merged tree (overlay + underlay) and emit a NEW EROFS
    /// image at the given path via `fs_erofs::mkfs::build_image`.
...
pub struct Mount {
    /// The opened EROFS volume. Read by:
...
impl Mount {
    /// Open an EROFS image. `partition` selects a 1-indexed partition
    /// in a whole-disk image; `None` or `Some(0)` means "treat `image`
    /// as the EROFS volume directly."
    ///
    ...
    /// Open the image as a single, unsliced EROFS volume.
    pub fn open_direct(image: &Path) -> Result<Self> {
        ...
        let fs = Filesystem::open(dev).map_err(|e| {
            anyhow!(
                "open EROFS at {}: {e}{}",
                image.display(),
    ...
    /// Open the Nth partition (1-indexed) inside `image` as an EROFS
    ...
    /// the merged tree and emits a fresh EROFS image at the configured
    /// path.
...
fn run_impl(_mount: Mount, _mount_point: &str) -> Result<()> {
    eprintln!("erofs: WinFsp mount only supported on Windows (with --features mount).");
    Ok(())
}
...
/// reflecting the post-overlay state, and emit a new EROFS image at
...
fn rebuild_image(fs: &Filesystem, overlay: &Overlay, out_path: &Path) -> Result<()> {
    ...
    let bytes = build_image(root, blkszbits).map_err(|e| anyhow!("rebuild EROFS image: {e}"))?;
...
mod winfsp_adapter {
    ...
    //! or used to rebuild a fresh EROFS image, controlled by the
    //! `DismountPolicy` configured at mount time.
    //!
    //! Path conversions: WinFsp gives backslash-separated UTF-16 paths
    //! (`\foo\bar`); EROFS's lookup_path wants slash-separated UTF-8
    //! (`/foo/bar`). Done in [`winpath_to_unix`].
    //!
    //! Time conversions: EROFS stores 64-bit unix-epoch seconds + nsec;
    ...
    /// Populate `FileInfo` from an EROFS `Inode`.
    ...
    fn populate_overlay_file_info(entry: &OverlayEntry, info: &mut FileInfo, read_only: bool) {
        ...
        // can't collide with a real EROFS NID. We use the high bit set
    ...
    impl ErofsContext {
        ...
        /// reader could see the pre-write content; but the EROFS
    ...
    impl FileSystemContext for ErofsContext {
        type FileContext = ErofsFileContext;
        ...
        // by resolving symlinks within the EROFS layer when explicitly
    ...
    /// Mount the given EROFS source on a Windows mount point.
    ///
    ...
    pub fn run(mount: Mount, mount_point: &str) -> Result<()> {
        ...
        // `ErofsContext`, so we must capture references first. Both
        ...
        let ctx = ErofsContext::new(mount)?;
        ...
        println!("erofs mounted at {mount_point} ({banner}). Ctrl-C to unmount.");
...
mod tests {
    ...
    /// Build a minimal valid EROFS image manually. Mirrors the layout
    /// used by `fs_erofs::fs::tests::build_image`:
    ...
    fn mount_open_part_zero_treated_as_direct() {
        ...
        // image as a single EROFS volume with no partition slicing.
    ...
    fn volume_info_math_matches_superblock() {
        ...
        // without needing to construct the full `ErofsContext` (which
