//! erofs-win-driver CLI entry point.
...
use erofs_win_driver::{mount, probe};
...
#[command(
    name = "erofs",
    about = "Browse and (eventually) mount EROFS volumes on Windows"
)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
...
#[derive(Args, Clone)]
struct ImgArg {
    /// EROFS filesystem image (file path).
    image: PathBuf,
}
...
enum Cmd {
    ...
    Watch,
    /// SCM service variant -- started by `sc start ErofsWatcher`.
    #[cfg(windows)]
    Service,
    ...
    Mount {
        ...
        /// underlay EROFS image is never touched.
        #[arg(long)]
        ro: bool,
        ...
        /// a NEW EROFS image at the given path. The original image is
...
fn open_fs(img: &PathBuf) -> Result<Filesystem> {
    ...
    Filesystem::open(dev).map_err(|e| anyhow!("open EROFS: {e}"))
}
...
/// Open the given disk/partition as an EROFS filesystem and mount it on
...
fn cmd_mount(
    ...
    let m = mount::Mount::open(&path, part)
        .with_context(|| format!("opening EROFS on {disk}"))?
