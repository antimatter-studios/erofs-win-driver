//! In-memory read-write overlay for the read-only EROFS mount.
//!
//! EROFS is fundamentally read-only at the format level — the on-disk
//! image carries no journal, no allocation bitmap, no free-space
//! bookkeeping. To present a writable Windows volume on top of an EROFS
//! image, the WinFsp adapter consults this `Overlay` BEFORE every read
//! and stages every mutation INTO it. The underlay (`fs_erofs::Filesystem`)
//! is never touched.
//!
//! Lookup precedence:
//! 1. Overlay `Hit(entry)` → entry's contents are returned (or, for
//!    `Deleted`, NotFound is propagated).
//! 2. Overlay `Miss` → fall through to the EROFS underlay.
//!
...
/// Read-write overlay layered atop the read-only EROFS underlay.
pub struct Overlay {
...
mod tests {
    ...
    //! tests don't touch `fs_erofs::Filesystem` at all. End-to-end
    //! merging with a real EROFS underlay lives in
