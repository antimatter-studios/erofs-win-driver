<#
.SYNOPSIS
  Build the erofs-win-driver installer (MSI + Burn bootstrapper EXE).

.DESCRIPTION
  Two-stage WiX 4 build:
    1. `wix build Product.wxs`   ->  erofs-win-driver-<ver>.msi
    2. `wix build Bundle.wxs`    ->  erofs-win-driver-<ver>-Setup.exe
...
.PARAMETER ExePath
  Path to the release erofs.exe. Required.
...
.PARAMETER Arch
  ...
  A mismatch (e.g. arm64 erofs.exe in an x64-templated MSI) installs but
...
$msiOut    = Join-Path $OutputDir "erofs-win-driver-$Version-$Arch.msi"
$bundleOut = Join-Path $OutputDir "erofs-win-driver-$Version-$Arch-Setup.exe"
