...
.DESCRIPTION
  ...
  Validation-Unattended-Failed. This script catches that locally + in CI:
    ...
    4. Asserts erofs.exe is present at the expected install path.

  Side-effect: leaves erofs-win-driver + WinFsp installed on the host. Run on
...
param(
  ...
  [string]$InstallDir = (Join-Path $env:ProgramFiles 'erofs-win-driver')
)
...
$erofs = Join-Path $InstallDir 'erofs.exe'
if (-not (Test-Path $erofs)) {
  throw "Setup.exe reported success but '$erofs' is missing -- install incomplete."
}

Write-Host "verify-silent: PASS -- '$erofs' present, exit $exit, no GUI hang."
