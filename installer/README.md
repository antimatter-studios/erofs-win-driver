# erofs-win-driver installer

WiX 7 source for the erofs-win-driver Windows installer. Two artefacts:

> **Upstream:** these wxs files are an erofs-customised copy of the
...
|-----------------------------------------------|-------------------------|-----------------------------------------------------------|
| `erofs-win-driver-<ver>-<arch>-Setup.exe`      | **end users (default)** | Burn bootstrapper: installs WinFsp first, then the MSI.   |
| `erofs-win-driver-<ver>-<arch>.msi`            | IT admins (SCCM/Intune) | Plain MSI; assumes WinFsp is already deployed separately. |

`<arch>` is `x64` or `arm64` — must match the `erofs.exe` you embed.

Both ship `erofs.exe`, `Mount-Erofs.ps1`, an Explorer right-click "Mount as
erofs" verb on `.img` files, and Start Menu shortcuts.
...
installer\build.ps1 -ExePath target\release\erofs.exe
...
- `erofs-win-driver-<ver>-<arch>-Setup.exe` — ship this.
- `erofs-win-driver-<ver>-<arch>.msi`        — admin channel only.

`build.ps1` parameters:

| Param         | Default                                                      |
|---------------|--------------------------------------------------------------|
| `-ExePath`    | *(required)* — path to the release `erofs.exe`                |
| `-Version`    | parsed from `Cargo.toml`                                     |
| `-OutputDir`  | `<repo>\dist`                                                |
| `-MsiOnly`    | switch — skip the bundle stage                               |
| `-Arch`       | `x64` (default) or `arm64` — must match `erofs.exe`           |

`-Arch` must match the embedded `erofs.exe`'s Rust target:
...
- Installs `erofs.exe`, `Mount-Erofs.ps1`, and `LICENSE.txt` into
  `%ProgramFiles%\erofs-win-driver\`.
- Appends the install dir to the **system** `PATH`.
- Adds Explorer right-click "Mount as erofs" on `.img` files (via
  `HKCR\SystemFileAssociations\.img\shell\MountAsErofs` — non-destructive;
  the built-in Disk Management "Mount" verb keeps working for VHD/ISO).
- Adds two Start Menu shortcuts under "erofs-win-driver":
  - **Mount erofs image...** — file picker → `erofs mount`.
  - **erofs watch service** — runs `erofs watch` in a console.
...
erofs-win-driver MSI. WinFsp is left in place on uninstall
...
```powershell
# Bundle install:
"%ProgramData%\Package Cache\{b6d4e8a1-3f29-4c7d-9e22-1a8c5d6f7e34}\erofs-win-driver-<ver>-Setup.exe" /uninstall

# MSI-only install:
msiexec /x dist\erofs-win-driver-<ver>.msi
```
...
- `Mount-Erofs.ps1` is also usable stand-alone — copy it next to a dev
  `erofs.exe` and run it.
