...
#
# Outputs:
#   ../rust-fs-erofs/test-disks/erofs-basic.img   — ~minimal tree
...
cd "${mkfs_root}"
cargo build --release --bin mkfs_erofs --quiet

if [[ ! -x "${mkfs_bin}" ]]; then
    echo "build-test-disks.sh: mkfs_erofs not built at ${mkfs_bin}" >&2
    exit 1
fi
...
build_basic() {
    local out="${disks_dir}/erofs-basic.img"
    local src
    src=$(mktemp -d -t erofs-basic.XXXXXX)
